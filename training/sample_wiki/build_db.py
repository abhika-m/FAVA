...
def cleanup_text(text):
    ...
    clean_text = re.sub(r"<ref\b[^>]*>.*?</ref>", "", text)
    return clean_text
    ...
    with open(filename) as f:
        for line in f:
            ...
            if not doc:
                continue
            # Add the document
            documents.append((normalize(doc['title']), doc['id'], doc['text'], len(doc['text'])))
    return documents
...
def store_contents(data_path, save_path, preprocess, num_workers=None):
    ...
    c = conn.cursor()
    c.execute("CREATE TABLE documents (title PRIMARY KEY, id, text, length);")
    ...
    db = DocDB(save_path)
    print("the total articles commited: ")
    print(len(db.get_doc_title()))
    print(db.get_doc_text("Canada"))
